...
def test_success_from_bin():
    ...
    excel_addin_generator.extractBinFromZip("success_bin.xlam")
    ...
    excel_addin_generator.extractBinFromZip("success_xlam.xlam")
    assert md5hasher.hash_file("tests/vbaProject.bin") == md5hasher.hash_file("xl/vbaProject.bin")
    
def test_not_bin_exception():
    """ Test that an exception is thrown if the bin file is not an OLE file"""
    with pytest.raises(Exception) as e_info:
        excel_addin_generator.createFromBin("tests/blank.bin", "src/data", "./fail.xlam")
        
def test_xlam_not_zip():
    """ Test that an exception is thrown if the zip is not a zip archive"""
    with pytest.raises(Exception) as e_info:
        excel_addin_generator.createFromZip("tests/blank.bin", "src/data", "./fail.xlam")

def test_main():
    excel_addin_generator.main(["./excelAddinGenerator", "./tests/vbaProject.bin", "success_bin.xlam"])
    excel_addin_generator.main(["./excelAddinGenerator", "success_bin.xlam", "success_xlam.xlam"])

def test_main_incorrect_type():
    """ Test that an exception is thrown if the zip is not a zip archive"""
    with pytest.raises(Exception) as e_info:
        excel_addin_generator.main(["./excelAddinGenerator", "./src/data/xl/styles.xml", "fail.xlam"])
